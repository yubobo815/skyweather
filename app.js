...
function renderWeather(place, forecast) {
  ...
  const todayUv = getUvDetails(daily.uv_index_max?.[0]);

  latestForecast = forecast;
  latestPlace = place;
  currentWeather.classList.remove("hidden");
  currentIcon.innerHTML = getWeatherIcon(currentInfo.type);
  currentAccessories.innerHTML = renderCurrentAccessoryBadges(todayAccessories);
  ...
  currentHumidity.innerHTML = renderMetricValue(`${current.relative_humidity_2m}%`);
  currentWind.innerHTML = renderMetricValue(`${Math.round(current.wind_speed_10m)} ${units.wind}`);
  ...
  currentUv.innerHTML = renderUvMetric(todayUv);
  currentUv.closest(".metric-card")?.setAttribute("data-level", todayUv.level);
...
function updateLocalTime(timeZone) {
  currentLocalTime.innerHTML = renderMetricValue(formatLocalTime(timeZone));
}
...
function renderRainMetric(daily, index) {
  ...
  return renderMetricValue(
    `${rainChance}%`,
    `${t("chance")} · ${getRainStrength(rainAmount)} · ${formatRainAmount(rainAmount)}`,
  );
}

function renderUvMetric(uv) {
  if (!uv.available) return renderMetricValue(uv.label);

  return `
    <span class="uv-recommendation" data-level="${escapeHtml(uv.level)}">
      <span class="uv-visual" aria-hidden="true">
        <strong>${escapeHtml(uv.displayScore)}</strong>
        <span>/10</span>
      </span>
      <span class="uv-copy">
        <strong>${escapeHtml(uv.label)}</strong>
        <small>${escapeHtml(t("currentDetails.uvIndex"))}</small>
      </span>
    </span>
  `;
}
...
function getUvDetails(value) {
  ...
  const score = Math.max(0, Math.round(numericValue));
  const displayScore = score > 10 ? "10+" : String(score);
...
function renderCurrentAccessoryBadges(accessories) {
  const badges = accessories
    .slice(0, 2)
    .map(
      (accessory) => `
        <span class="weather-accessory today-accessory" data-accessory="${accessory.type}" title="${escapeHtml(accessory.label)}">
          ${getAccessoryIcon(accessory.type)}
  ...
  return `
    <span class="today-kit-label">${escapeHtml(t("todayKit"))}</span>
    ${badges}
  `;
}
